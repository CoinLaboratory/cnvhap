...
public class rtest {
	...
	if (!Rengine.versionCheck()) {
		...
		Rengine re=new Rengine(args, false,null);//new TextConsole());
		...
		try {
			...
			re.eval("z = array(0,c(2,2))");
			re.assign("tmp", new double[] {1,1});
			re.eval("z[1,] = tmp");
			REXP z1 = re.eval("z");
			re.eval("summ= summary(lm(y~x))");
			...
			int ind = Arrays.asList(names.at(0).asStringArray()).indexOf("x");
			String[] names1 = names.at(1).asStringArray();
			//names.asList();
			//System.out.println(  x =re.eval("x") );
			//System.out.println(  y1 = re.eval("y=(1:10)*0.2"));
			//System.out.println(z = re.eval("summary(lm(y~x))"));
			//String[] keys = coeff.asVector();
