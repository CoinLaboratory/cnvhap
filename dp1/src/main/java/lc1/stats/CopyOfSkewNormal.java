...
public class CopyOfSkewNormal extends ProbabilityDistribution implements lc1.stats.ProbabilityDistribution, MultivariateFunction, Comparable{
   int paramIndex = 1;
  // String id;
   
   CopyOfSkewNormal prior;
    ...
    public CopyOfSkewNormal clone(){
        return new CopyOfSkewNormal(this);
    }
    ...
    public void setPriors(lc1.stats.ProbabilityDistribution distx, int type){
    	CopyOfSkewNormal dist1 = (CopyOfSkewNormal)distx;
    	if(type==1)this.prior.scale = dist1.scale;
    	else if (type==2) this.prior.shape = dist1.shape;
    	else if(type==0) this.prior.location = dist1.location;
		//this.dist[0].setPriors(((Mixture)distx).dist[0]);
	}
    public CopyOfSkewNormal clone(double u){
        return new CopyOfSkewNormal(this, u);
    }
    public void setPriorVar(double e){
		//this.stddevPrior[0] = e;
		this.scale = e;
		this.prior.scale = e;
	}
    public double prior(){
    	return this.normal.logpdf(location, prior.location, 1/pseudoM) +
    	      this.normal.logpdf(Math.log(scale), Math.log(prior.scale), 1/pseudoSD)+
    	      this.normal.logpdf(shape, shape, 1/this.pseudoSk);
    ...
    public void addCounts(lc1.stats.ProbabilityDistribution probabilityDistribution){
        CopyOfSkewNormal n = (CopyOfSkewNormal) probabilityDistribution;
    ...
    public static void main(String[] args){
       ...
       CopyOfSkewNormal sn = new CopyOfSkewNormal(0.0,0.05,1e10, -5, 5, 100, 1);
      // SkewNormal sn1 = new TrainableNormal(0.0,0.05,1e10, -5, 5, 100, 1);
       ...
       System.err.println();
        ...
        sn = new CopyOfSkewNormal(-1.0,0.2,0, -5, 5, 100,1);
       for(int i=0; i<x.length; i++){
       System.err.print(sn.dsn(x[i], false)+"\t");
       }
       System.err.println();
       
       sn = new CopyOfSkewNormal(-1.0,0.2,10, -5, 5, 100,1);
       for(int i=0; i<x.length; i++){
       System.err.print(sn.dsn(x[i], false)+"\t");
       }
       System.err.println();
    ...
    public boolean equals(Object obj){
        return this.name.equals( ((CopyOfSkewNormal)obj).name);
    }
    public int hashCode(){
...
public CopyOfSkewNormal(double mean, double stddev, double skew, double min, double max, double round, double priorModifier) {
    this(null, mean, stddev, skew, new double[] {min, 0.001, -1e10},  new double[] {max, 1e3, 1e10}, round, priorModifier);
 }

public CopyOfSkewNormal(String name, double mean, double stddev, double skew, double min, double max, double round, double priorModifier) {
...
public void setParams(double mean, double stddev, double skew) {
    ...
    this.scale = stddev;
    this.shape = skew;
   if(prior!=null){ this.prior.location = mean;
    this.prior.scale = stddev;
    this.prior.shape = skew;
   }
}
final public String id; //
    public CopyOfSkewNormal(String name1, double mean, double stddev, double skew, double[] lower , double[] upper, double round, double priorModifier){
       this.priorModifier = priorModifier;
        this.round = round;
        this.lower =lower;
        this.upper = upper;
         this.id = name1;
        if(!name1.endsWith("p")) this.prior = new CopyOfSkewNormal(name1+"_p", mean, stddev, skew, null,null, round, priorModifier);
     setParams(mean, stddev, skew);
    this.recalcName();
        if(stddev<0) throw new RuntimeException("!!");
    }
    public CopyOfSkewNormal(String name1, double mean, double stddev,double stddevprior, double skew, double[] lower , double[] upper, double round, double priorModifier){
        this.priorModifier = priorModifier;
         this.round = round;
         this.lower =lower;
         this.upper = upper;
         this.id = name1;
      setParams(mean, stddev, skew);
      this.prior.scale = stddevprior;
     this.recalcName();
         if(stddev<0) throw new RuntimeException("!!");
     }
   
    public CopyOfSkewNormal(CopyOfSkewNormal skewNormal) {
     ...
     setParams(skewNormal.location, skewNormal.scale, skewNormal.shape);
     this.name = skewNormal.name;
   obsx = new ArrayList<Double>();
   obsv = new ArrayList<Double>();
   this.prior = skewNormal.prior;
    }
    
    
    public CopyOfSkewNormal(CopyOfSkewNormal skewNormal, double u) {
      this(skewNormal);
     this.location =   normal.quantile(Constants.rand.nextDouble(),location, 1.0/u);
     this.prior = skewNormal.prior.clone();
    }
    ...
    public void setParamsAsAverageOf(lc1.stats.ProbabilityDistribution[] tmp) {
       ...
       for(int i=0; i<tmp.length; i++){
           location += ((CopyOfSkewNormal)tmp[i]).location;
           this.scale += ((CopyOfSkewNormal)tmp[i]).scale;
           this.shape += ((CopyOfSkewNormal)tmp[i]).shape;
       }
   ...
   /* public double dsn (double x, boolean log)
    {
     ...
     double y;
      if(log)
        ...
        if(y>1){
        	double y1 = 2*normal.cdf(z*shape, 0, 1)*normal.pdf(z, 0, 1);
        	throw new RuntimeException("!!");
        }
      }
...
//      replace(y, scale<= 0, NaN)
    ...
    public double dsn(double x, boolean log){
    	 double z  =  (x-location)/scale;
    	 if(log){
    		 return Math.log(2*normal.cdf(z*shape, 0, 1)) + normal.logpdf(z, 0, 1);
    	 }
    	 else return 2*normal.cdf(z*shape, 0, 1)*normal.pdf(z, 0, 1);
 ...
 double pseudo) {
    	...
    	if(Math.abs(pseudo)>0){
    		...
    		int k = numObs+i;
	   		 ...
	   		 y.setQuick(k,0, v*(pseudo < 0  ? this.prior.location : this.location ));
	   		i++;
    	}
    	 return i;
	}
    ...
    public int fillVariance( DoubleMatrix2D y, int numObs, double pseudo) {
    	int i;
    	 ...
    	 if(Math.abs(pseudo)>0){
    		 ...
    		 y.setQuick(k,0, v*( Constants.transformVariance(pseudo<0 ? this.prior.scale : this.scale)));
    		 i++;
    	 }
    	 return i;
	}
    ...
    static int numToSample = 10;
    public void maximise(double pseudoM, double pseudoSd, double pseudoSk) {
    	if(!Constants.trainSkew() || (pseudoM>1e5 )) return ;
    	
    //	if(true) throw new RuntimeException("!!");
        if(this.sum() < Constants.trainThresh() ) return;
        if(numToSample>0 && false){
        //	double[] weights = new double[numToSample];
       sum+=this.prior.sample(numToSample, this.obsx,pseudoM, this.obsv);
     // sum+=//numToSample;
        }
        cnt=0;
        this.pseudoM = pseudoM;
      ...
      //  setPrior(pseudoM, pseudoSd, pseudoSk);
            ...
            //   Logger.global.info(this.getObsString());
               // final ConjugateDirectionSearch os = new ConjugateDirectionSearch();
                ...
                final double[] xvec =  new double[] {location, Math.log(scale), shape};
                final double[] init = new double[xvec.length];
                System.arraycopy(xvec, 0, init, 0, xvec.length);
            	/*double min = os1.findMinimum(CopyOfSkewNormal.this,xvec, 2, 2);*/
            	
                Runnable run = new Runnable(){
                public void run(){
                	try{
                		double min = os1.findMinimum(CopyOfSkewNormal.this,xvec, 1, 1);
                	}catch(Exception exc){
                		exc.printStackTrace();
                		os1.optimize(CopyOfSkewNormal.this,xvec, 0.01, 0.01);
                	}
                }
                };
  ...
  /*  public void setPrior(SkewNormal bground,double pseudo, double pseudoSD, double pseudoS  ){
   ...
   public void addSamples(double weight, int num){
	   for(int k=0; k<num; k++){
		   double v = this.sample();
	   }
   }
    ...
    public double calcLH(){
        ...
        for(int i=0; i<obsx.size(); i++){
         ...
         //    if(weight>1e-5){
                 double lprob = this.dsn(val, true);
              /*   if(lprob>=0){
                 	throw new RuntimeException("l is zero");
                 }*/
              /* if(prob==Double.NEGATIVE_INFINITY){
                  //  System.err.println("doubleNeg");
                   double res =  -1e3*(
                              Math.pow(location - this.prior.location, 2) + 
                              Math.pow(scale -  this.prior.scale, 2)+
                              Math.pow(shape - this.prior.shape, 2)
                            );
                   return res;
                }*/
                 l+=lprob*weight;
...
//      replace(p, scale<= 0, NaN)
  ...
  int cnt=0;
   ...
   //    if(true) System.exit(0);
       ...
       this.scale = Math.exp( argument[1]);
       this.shape = argument[2];
     
       double res = -1*(calcLH()+prior());
      // System.err.println(cnt);
       cnt++;
       if(cnt>100000){
    	   throw new RuntimeException("too many its");
       }
     //System.err.println(cnt+" eval "+argument[0]+" "+argument[1]+" "+argument[2]+" "+res);
       return res;
    }
  ...
  //  @Override
    ...
    public int  sample(int num,List<Double> res, double pseudoM, List<Double> obsv) {
    	
    	SortedSet<Double> pvals = new TreeSet<Double>();
    	for(int k=0; k<num; k++){
    		pvals.add(Constants.rand.nextDouble());
    	}
    	double sum2 =0;
    	int sze = obsv.size();
    	//int cnt=0;
    	double low =pvals.first();
    		 double sum =0;
    	        double minq = 1e-7;
    	        double maxq = (1.0-1e-7) ;
    	        double min = normal.quantile(minq, location, scale);
    	       ...
    	       inner: for(double j =min; j<max; j+=incr){
    	            double probj = this.probability(j)/sum1;
    	            sum+=probj;
    	            while(sum>=low){
    	            	res.add(j);
    	        ...
    	        int cnt1 = num - pvals.size();
    	        for(int k=0; k<cnt1; k++){
    	        	
    	        	obsv.set(k+sze, obsv.get(k+sze) * (pseudoM/sum2));
    	        }
    	        return cnt1;
    	       /* for(int k2=0; k2<pvals.size(); k2++){
    	        	pvals.add(max);
    	        }*/
...
//      u1[id] <- (-u1[id])
   ...
   /* public Map<Double, Double> getObservations() {
    ...
    public void transferParams(ProbabilityDistribution pdist0){
        this.location = ((CopyOfSkewNormal)pdist0).location;
        this.scale = ((CopyOfSkewNormal)pdist0).scale;
        this.shape = ((CopyOfSkewNormal)pdist0).shape;
    }
	...
	public int compareTo(Object o) {
		return this.name.compareTo(((CopyOfSkewNormal)o).name);
	}
