...
public class ColorAdapter{
   ...
   static List<Color> defaultC = new ArrayList<Color>();
   {
    ...
    public ColorAdapter(HaplotypeHMM hmm) {
    ...
    private static Color[] getColors(HaplotypeHMM hmm) {
		 Color[] colors = new Color[hmm.modelLength()];
		  colors[0] = Color.GRAY;
		 ...
		 // int[] count = new int[colo.length];
		  for(int i=1; i<colors.length; i++){
			EmissionState st = (EmissionState)  hmm.getState(i);
			Integer nocop = st.noCop();
    ...
    private List<Color> randomise(List<Color> defaultC2) {
        ...
        while(defaultC2.size()>0){
            ne.add(defaultC2.remove(Constants.nextInt(defaultC2.size())));
        }
        return ne;
    }
   ...
   static  int no_states;
	...
	public static ColorAdapter get(MarkovModel hmm) {
		// TODO Auto-generated method stub
		if(ca==null || no_states!=hmm.modelLength()) {
			ca = new ColorAdapter((HaplotypeHMM)hmm);
