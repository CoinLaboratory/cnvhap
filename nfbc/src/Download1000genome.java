...
public class Download1000genome {
  ...
  static PrintWriter pw;
    ...
    public static void main(String args[]){
        try{
        	pw = new PrintWriter(new BufferedWriter(new FileWriter("todownload")));
            getFileName();
            for(int i=0; i<dirName.size(); i++){
                download(dirName.get(i).split("/")[2], dirName.get(i).split("/")[4]);
            }
            pw.close();
        }catch(Exception exc){
            exc.printStackTrace();
...
//                10_5000001_10000001
    ...
    public static void getFileName()throws Exception{
          BufferedReader br = new BufferedReader(new FileReader(new File("pilot_data.alignment.index")));
          String st ;
          while((st = br.readLine())!=null){
              String[] str = st.split("\\s+");
              if(str[0].contains("SLX.maq.SRP000031") && !str[0].contains(".unmapped.")){
                  dirName.add(str[0]);
              }
          }
          br.close();
    ...
    public static void download(String id, String name) throws Exception{
